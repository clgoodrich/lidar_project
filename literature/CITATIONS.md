...
### Axelsson 2000 — Progressive TIN densification, the ground filter this delivery used
- **Citation:** Axelsson, P. (2000). "DEM Generation from Laser Scanner Data Using Adaptive TIN Models." *International Archives of Photogrammetry and Remote Sensing* 33(B4): 111-118.
- **About:** The progressive TIN densification ground filter. A sparse TIN is seeded on the lowest point in each cell of a coarse grid, then points are added iteratively if their distance to the enclosing triangle facet and their angle to the nearest facet vertex both fall under thresholds that tighten as the surface converges. It is the algorithm behind TerraScan's ground routine, LAStools `lasground`, and the family PDAL's SMRF and PMF belong to, so it is almost certainly what produced class 2 in the USGS 3DEP delivery.
- **Used for:** Framing the failure mode tested in the pit-depth experiment. The seed grid is coarser than a well pit, so a pit that never earns a seed can be spanned by the TIN, leaving genuine floor returns below the surface and unclassified. **The test found this is NOT happening in this delivery** — ground density inside annotated pit floors is 92% of the surrounding ring, and none of 216 floors is empty of class 2. The citation is retained because it is what made the hypothesis worth testing and what the negative result is stated against.
- **Generated:** `notebooks/wellsight_v2/s7_analysis/_reclaim_ground_pit_depth_experiment.py`; `docs/iterations/ground_reclassification_pit_depth.md`; `data/9t/results/ground_reclassification/`.
- **Source:** https://www.isprs.org/proceedings/XXXIII/congress/part4/110_XXXIII-part4.pdf — cite-only (ISPRS archive, no local copy retained).

### Pingel, Clarke & McBride 2013 — SMRF, the ground filter we ran ourselves
- **Citation:** Pingel, T.J., Clarke, K.C., McBride, W.A. (2013). "An improved simple morphological filter for the terrain classification of airborne LIDAR data." *ISPRS Journal of Photogrammetry and Remote Sensing* 77: 21-30. doi:10.1016/j.isprsjprs.2012.12.002.
- **About:** The Simple Morphological Filter. A minimum-elevation surface is rasterised at `cell` resolution, opened with a linearly growing window up to `window`, and a cell is called ground when its elevation sits within a threshold that grows with the local `slope` — `threshold + slope * window_size * cell`. Holes in the provisional surface are filled by inpainting, then the original points are compared against it. Its argument against the TIN-densification family is that it needs one primary parameter (`slope`) rather than a coupled set, so the result is stable across terrain rather than tuned per site. It is implemented as PDAL `filters.smrf`.
- **Used for:** Reclassifying ground ourselves on two Venango tiles with no scan-angle rule, to measure what the vendor's 18-degree cut cost. Parameters `cell` 1.0, `slope` 0.35, `window` 18.0, `threshold` 0.5, `scalar` 1.25, `ignore Classification[7:7]`. The paper's stability claim was checked rather than assumed: sweeping `slope` over 0.15-0.70 moves the recovered-point count by 0.9% and vendor agreement by 0.06%, so 0.35 was taken as the middle of a flat curve. Result: 1.45 M and 1.62 M points recovered per tile, DEM void rate 15.99% -> 9.97% and 12.36% -> 6.27%, with movement confined to cells the vendor had no ground in (0.3% of previously-covered cells differ by more than 10 cm).
- **Generated:** `notebooks/wellsight_v2/s7_analysis/_smrf_reclassify_ground.py`; `docs/iterations/smrf_ground_reclassification.md`; `data/9t/results/smrf_ground/`; `docs/presentation/figures_30to45min/1_data_qa/smrf_ground/`.
- **Source:** https://doi.org/10.1016/j.isprsjprs.2012.12.002 — cite-only (Elsevier, paywalled).

### USGS 2020 — 3DEP Lidar Base Specification
- **Citation:** Heidemann, H.K. (2018, rev. 2020). "Lidar Base Specification." *U.S. Geological Survey Techniques and Methods*, book 11, chap. B4, version 2020 rev. A. doi:10.3133/tm11B4.
- **About:** The acquisition and deliverable standard every USGS 3DEP dataset is flown and processed to. Defines the Quality Levels (QL1 ~8 pts/m2, QL2 ~2 pts/m2 nominal), the required vertical accuracy (non-vegetated vertical accuracy RMSEz <= 10 cm for QL1 and QL2, 19.6 cm at 95% confidence), the ASPRS classification codes a delivery must use, and the convention that vegetation is split low / medium / high at 2 m and 5 m above ground.
- **Used for:** Two things. The ASPRS 3 / 4 / 5 height bands used to reclassify the 43.7% of returns the PA WesternPA 2019 D20 delivery leaves unassigned. And the RMSEz <= 10 cm bar the excluded wide-angle returns were measured against -- they come in at 0.065-0.067 m, inside spec, which is what makes the vendor's 18-degree ground-classification cut unjustifiable rather than merely conservative.
- **Generated:** `notebooks/wellsight_v2/s7_analysis/_classify_nonground_returns_9t.py`; `notebooks/wellsight_v2/s7_analysis/_are_excluded_returns_accurate.py`; `notebooks/wellsight_v2/s7_analysis/_scan_angle_cliff_across_acquisitions.py`; `docs/iterations/nonground_classification_and_scan_angle_cut.md`.
- **Source:** https://doi.org/10.3133/tm11B4 — cite-only (USGS publication, not retained locally).


### RIEGL VQ-1560 series — the sensor that flew 9t, and why its scan lines are straight
- **Citation:** RIEGL Laser Measurement Systems GmbH. "RIEGL VQ-1560i / VQ-1560 II" dual-channel airborne laser scanning system — datasheet and scan-pattern description ("cross-fire").
- **About:** A dual-channel airborne scanner built on a **rotating polygon mirror wheel**, not an oscillating mirror. Each channel lays down *straight, parallel* scan lines. The two channels' line sets are tilted 28 degrees against each other (plus and minus 14) with a forward/backward look of plus and minus 8 degrees at the swath edge, which RIEGL markets as the "cross-fire" pattern and presents as giving an even ground distribution that is invariant to terrain height. The datasheet explicitly contrasts the wheel — tilted straight scan lines, optimum interleaving independent of acquisition parameters — against oscillating-mirror systems, which produce interleaving zig-zag or sinusoidal scans and are "prone to unfavorable phase conditions".
- **Used for:** Identifying the scanner behind the PA WesternPA 2019 D20 delivery (header `generating_software` is "QSI LiDAR Suite"; the sensor is not named in the LAS header) and, through that, correcting the explanation of the CHM corn rows. It settles three things the delivered file alone could not: the lines are straight and parallel rather than a zig-zag, so there is **no turnaround and no phase relationship between successive lines**; two channels are interleaved in the returns, which is what made `scan_angle` appear to reverse and produced a spurious "oscillating mirror, 54.3 Hz" reading; and the roughly 8-degree offset measured between the scan-line bearing (78.0 deg) and across-track (85.8 deg) matches the datasheet's forward/backward look. The corn rows lie **along the scan lines** — 79.0 deg measured on the NoData mask against 78.0 deg measured on the ground point pattern.
- **Generated:** `notebooks/wellsight_v2/s7_analysis/_measure_scanner_geometry_9t.py`; `data/9t/results/nonground_classification/scanner_geometry_9t.json`; `docs/presentation/viewers/why_the_corn_rows.html`; corrections to `_test_corduroy_is_scan_geometry_9t.py`, `_test_corduroy_vs_vendor_cut_9t.py`, `_build_chm_nodata_cross_section_9t.py`, `_build_chm_nodata_row_vs_ground_9t.py`.
- **Source:** http://www.riegl.com/products/airborne-scanning/riegl-vq-1560-ii/ — cite-only (manufacturer datasheet). Sensor identification supplied by the user from the manufacturer's published scan-pattern description; it is not recorded in the LAS header, so it is an attribution from the specification rather than a measurement.


### Schreiber & Schmitz 1996 — IAAFT surrogates, the null for 9t change patches
- **Citation:** Schreiber, T. & Schmitz, A. (1996). "Improved Surrogate Data for Nonlinearity Tests." *Physical Review Letters* 77(4): 635-638. doi:10.1103/PhysRevLett.77.635. arXiv:chao-dyn/9909041.
- **About:** The iterative amplitude-adjusted Fourier transform (IAAFT). A surrogate is built by alternating two steps. One imposes the data's Fourier amplitude spectrum, which fixes the autocorrelation. The other rank-remaps the values onto the data's own sorted values, which fixes the distribution. The result keeps both and scrambles everything else.
