...
**Last updated:** 2026-05-20, after iter 05.
...
Per-pixel max of pit-class probs from both models, then re-normalized. Highest mean per-pit IoU yet (0.677) but pixel IoUs drop slightly because it over-predicts where either model is confident. Specifically, it manufactures ~69 elongated road-shoulder false positives that the mean ensemble avoids — quantified by the diagnostic at `data/derivatives/9t/diagnostics/elongated_fps_maxpit/`.
