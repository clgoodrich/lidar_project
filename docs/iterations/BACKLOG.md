...
- **Do the same 5-fold treatment for pads.** **Script written and dry-checked 2026-07-28**, not yet run (deferred, user needed the machine). `notebooks/wellsight_v2/plats/_pad_unet_cv5.py`, folds balanced at 129–132 pads across 650 in-tile pads / 129 blocks. Parameters copied verbatim from `_plat_unet.py` (patch 384, jitter 40 m, 2 classes, focal 0.15/0.85) so it measures the split, not a new model. No checkpoints exist yet, so all five folds train from scratch: est. 75–100 min GPU, ~1 GB disk. Resumable fold-by-fold. Launch with `python notebooks/wellsight_v2/plats/_pad_unet_cv5.py --folds 5 --epochs 40`.
