...
- **Test n / dataset-era mixing** (corrected 2026-07-01): the 20-pit/9-pad test sets belong to the pre-2026-06-10 dataset; current manifests hold 65 test pits / 93 test pads. But the instance rows on the LEADERBOARD still come from the old-era evals (stale `test_metrics.json`), while U-Net rows use the new split — re-run instance evals on the current split, and report binomial CIs either way.
