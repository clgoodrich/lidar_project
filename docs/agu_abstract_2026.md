...
Orphan and abandoned petroleum wells are an ongoing problem across North America. During the 19th century, thousands of wells were drilled in western Pennsylvania, representing the North American oil boom. This boom was wild, reckless and poorly documented, leaving the hills and valleys riddled with potentially dangerous wells leaking methane and brine into forests and streams. Given the heavy vegetation in the area, locating these wells involves countless hours on foot or reliance on landowners or hikers. To facilitate well detection, we present a lidar-based framework to identify the surface expressions of orphaned wells. Airborne lidar resolves the ground beneath the dense deciduous canopy, revealing terrain that optical imagery cannot capture. We process US Geological Survey 3D Elevation Program data from 2019 over this region into 0.5 and 1 meter resolution terrain models. For each model we derive a stack of terrain channels including local relief models, topographic openness, and slope residuals. Older wells leave three recurring signs: graded pads marking the site, access roads, and shallow depressions. We segment all three with U-Nets on this stack, from 471 annotated pits, 650 pads, 153 kilometers of road, and 37 kilometers of drainage as negatives. Scoring uses hand-drawn annotation withheld from training, plus detections checked by hand. Thresholds are selected on validation and test scored once. Five-fold cross-validation scores every feature in the first survey block by a model that never saw it. The models locate 93 percent of withheld pits and 94 percent of withheld pads, at precision 0.72 and 0.90. Roads are scored by length, recovering 98 percent of 735 withheld segments, and 82 percent of extracted road is real. It claims 3 percent of held-out drainage as road. We are extending this protocol to more blocks across Pennsylvania.
