...
2. Data resources and environment are defined as follows. Adhere strictly:
   ...
   * **Extensive documentation (mandatory):** Every meaningful piece of work in this project must be documented as durable prose, not just code + commit messages. This is non-negotiable.
     - **Per-iteration READMEs.** Every `notebooks/wellsight/**/iter_XX_*/` folder gets a `README.md` describing: the hypothesis or goal of the iteration, what was changed (data, model, loss, features, etc.), exact parameters used, training/inference results with metric tables, honest interpretation including failure modes, and what the next iteration would do. Treat each iteration like a mini lab notebook entry.
     - **Per-experiment write-ups under `docs/iterations/`.** A copy / expanded version of the iteration README also lives under `docs/iterations/iter_XX_<short_name>.md` for high-visibility browsability outside the code tree.
     - **Running leaderboard at `docs/iterations/LEADERBOARD.md`.** Updated every iteration with a comparison table covering all major metrics on the held-out test set (per-pit IoU mean/median, detection rate, IoU>0.3 count, pixel IoU per class, val miou, params used). Always show all prior iterations alongside the latest so trade-offs are visible at a glance.
     - **Pipeline / dataset-build docs under `docs/pipelines/`.** Any new data-prep, feature-build, or annotation-prep script gets a doc explaining what it produces, what assumptions it bakes in, and where its outputs sit on disk.
     - **Decision logs.** When a non-trivial choice is made (architecture, hyperparameter, train/val split scheme, etc.), record the reasoning in the relevant doc — not just the outcome. Future readers (including future-you) need the *why*.
     - **Honest tradeoffs.** Never write a doc that only celebrates the win. If an iteration regressed on some metric, name it explicitly. If a model is "best overall" but loses on detection rate, say so in the leaderboard and the iteration README.
     - **Cross-link liberally** between docs (`[iter 02](iter_02_smp_pretrained.md)`) so a reader can navigate the trajectory of the project without reading source.
