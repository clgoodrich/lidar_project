...
2. Data resources and environment are defined as follows. Adhere strictly:
   ...
   * **Descriptive-filename rule (enforce strictly):** Every file you generate must be named so that its *content* is obvious from the filename alone, without opening it or reading a doc. The name must carry: **what it shows**, **any parameter that defines it** (threshold, resolution, date, class), and **the location//dataset it covers**. Keep the existing location+resolution suffix convention (`_9t_05`, `_613590_1m`) and put the distinguishing detail *before* it. Use `0p20` style for decimals so the name stays shell- and GDAL-safe.
     - Good: `pit_unet_floor_mask_thr0p20_9t_05.tif`, `dod_9t_nonerosional_2m.tif`, `heldout_pit_unet_v2_found_vs_missed.gpkg`
     - Bad: `output.tif`, `mask2.tif`, `pit_prob.tif` (which model? which class? which threshold?), `test.gpkg`
     A reader six months from now — or a reviewer with only the file listing — must be able to tell two similar products apart. If two files differ only by a parameter, that parameter MUST appear in both names. This rule is in addition to the Full-path rule: name the file well *and* report its full path.
