# FINESST — Final Documents

The single latest version of every submission component, in one folder.
Assemble into the two NSPIRES upload PDFs in this order:

**PDF 1 — Anonymized Technical Proposal**
- `A1_proposal_STM` — Science/Technical/Management + References
- `A2_OSDMP` — Open Science & Data Management Plan
- `A3_mentoring_plan` — Mentoring Plan (unsigned)

**PDF 2 — Expertise & Resources (NOT anonymized)**
- `B1_research_readiness_statement`
- `B2_biosketch_FI_goodrich`
- `B3_biosketch_PI_glennie` — **missing (needs Glennie)**
- `B4_current_pending_FI_goodrich`
- `B5_current_pending_PI_glennie` — **missing (needs Glennie)**
- `B6_facilities_acknowledgements_budget`

## Still to finish before upload
- **Glennie's** biosketch (B3) and Current & Pending (B5) — B5 must list his
  pending NSF 25-526 proposal (overlapping MDV work).
- **Sign + date** B2 (FI biosketch) and B4 (FI C&P).
- **Budget amounts** in B6 (Amount column is blank).
- **Timing mismatch**: B2 says graduation 08/2029, B4's FINESST entry ends 12/2029 —
  align one of them.
  (B1 brackets are filled — v4 from the FI's edited source; B4 has no brackets;
  B6 GPU is correct: NVIDIA GeForce GTX 1070 Ti, 8 GB; FI ORCID is on file.)

Status 2026-07-09: A1 is the **accepted v10** S/T/M (Glennie comments #1–#9
resolved; H1/H2 stated; 12-pt TNR). A2/A3/B1/B6 converted to 12-pt TNR,
page-limits verified. B4 carries the stream-corridor scope fix. Project Summary
draft for the NSPIRES cover page: `barlow/docs/finesst_project_summary.md`.
