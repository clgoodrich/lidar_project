...
| 5 | Figure 2: add ICESat-2 / current NASA instruments; NCALM unmentioned | Caption updated: co-registration **to airborne-lidar and ICESat-2 control**; O2 relabeled "stream-corridor." **The figure IMAGE still needs a redraw** (add an ICESat-2 box, relabel O2) — flagged, can't redraw here. |
...
## Still needs a human
- **#1** is a real scope decision — v8 narrows to the corridor per his steer; confirm that's the intended call (vs. defending the whole-valley scope).
- **#5** the Figure 2 image must be regenerated (ICESat-2 box, O2 relabel) — the source figure script is needed.
