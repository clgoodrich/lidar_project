"""Merge 3x3 USGS_LPC_PA_WesternPA_2019 tiles around 621594 and build the
standard 0.5 m derivative stack over the resulting 4.5 km x 4.5 km extent.

Source tiles (all in data/files/, EPSG:6346 already):
  NW 619596   N 621596   NE 622596
  W  619594   .  621594  E  622594        <- center is our existing output3
  SW 619593   S 621593   SE 622593

Outputs:
  data/files/output3_9tile.las           (merged 9-tile LAS)
  data/derivatives/dem_9t_05.tif         (TIN over ground)
  data/derivatives/dsm_9t_05.tif
  data/derivatives/chm_9t_05.tif
  data/derivatives/ground_density_9t_05.tif
  data/derivatives/intensity_ground_9t_05.tif
  data/derivatives/hillshade_9t_05.tif
  data/derivatives/slope_9t_05.tif
  data/derivatives/roughness_11_9t_05.tif
  data/derivatives/local_relief_10_9t_05.tif
  data/derivatives/lrm_{5,11,25,51}_9t_05.tif
  data/derivatives/tpi_{05,15,51}_9t_05.tif, tpi_grad_{mag,dir}_9t_05.tif
  data/derivatives/openness_{pos,neg}_9t_05.tif
  data/derivatives/tile_overview_9t_05.png
"""
...
CODES = ['619593', '619594', '619596',
         '621593', '621594', '621596',
         '622593', '622594', '622596']
SRC = [DATA / f'USGS_LPC_PA_WesternPA_2019_D20_17TPF{c}.laz' for c in CODES]
for f in SRC: assert f.exists(), f
LAS_OUT = DATA / 'output3_9tile.las'

# extent of the 3x3 grid
X0, Y0, X1, Y1 = 619500.0, 4593000.0, 624000.0, 4597500.0
RES = 0.5
W = int((X1 - X0) / RES); H = int((Y1 - Y0) / RES)
T = from_origin(X0, Y1, RES, RES)
CRS = 'EPSG:6346'
print(f'grid: {W} x {H} cells @ {RES} m, extent ({X0},{Y0})-({X1},{Y1})')

def run_pipeline(pl, label, timeout=2400):
...
# =============================================================================
# (1) Merge 9 LAZ tiles -> one LAS
# =============================================================================
print('\n=== merging 9 LAZ tiles -> output3_9tile.las ===')
stages = [{'type': 'readers.las', 'filename': str(p)} for p in SRC]
stages.append({'type': 'filters.merge'})
stages.append({'type': 'writers.las', 'filename': str(LAS_OUT),
               'minor_version': 4, 'dataformat_id': 7,
               'forward': 'all', 'compression': 'false'})
run_pipeline({'pipeline': stages}, 'merge_9tile', timeout=3600)
print(f'merged LAS: {LAS_OUT}  size {LAS_OUT.stat().st_size/1e9:.2f} GB')
...
run_pipeline({'pipeline': [
    ...
    {'type': 'writers.raster', 'filename': str(DERIV / 'dem_9t_05.tif'), 'data_type': 'float32'},
]}, 'dem_9t_05', timeout=3600)
dem = read_tif('dem_9t_05.tif')
print(f'DEM 9t: nan {100*np.isnan(dem).mean():.2f}%   z {np.nanmin(dem):.1f}..{np.nanmax(dem):.1f}')

print('\n=== DSM (first returns) + CHM ===')
run_pipeline({'pipeline': [
    {'type': 'readers.las', 'filename': str(LAS_OUT)},
    {'type': 'filters.range', 'limits': 'ReturnNumber[1:1]'},
    {'type': 'writers.gdal', 'filename': str(DERIV / 'dsm_9t_05.tif'),
     'output_type': 'max', 'resolution': RES,
     'origin_x': X0, 'origin_y': Y0, 'width': W, 'height': H,
     'data_type': 'float32'},
]}, 'dsm_9t_05', timeout=3600)
dsm = read_tif('dsm_9t_05.tif')
chm = np.where(np.isnan(dsm) | np.isnan(dem), np.nan,
               np.maximum(dsm - dem, 0)).astype(np.float32)
write_tif('chm_9t_05.tif', chm)


# =============================================================================
# (3) Density + intensity (chunked read for memory)
# =============================================================================
print('\n=== density + intensity ===')
...
write_tif('ground_density_9t_05.tif', density, dtype='uint16', nd=0)
iv = ints[gmask][ok]
...
write_tif('intensity_ground_9t_05.tif', mean_i.reshape(H, W))
...
wbt.hillshade(dem='dem_9t_05.tif', output='hillshade_9t_05.tif', azimuth=315.0, altitude=45.0)
wbt.slope(   dem='dem_9t_05.tif', output='slope_9t_05.tif', units='degrees')
print('wrote hillshade_9t_05 + slope_9t_05')
...
write_tif('roughness_11_9t_05.tif', roughness)

rk = disk_kernel(10 / RES)
local_relief = (nanmax_disk(dem, rk) - nanmin_disk(dem, rk)).astype(np.float32)
write_tif('local_relief_10_9t_05.tif', local_relief)
...
for size in (5, 11, 25, 51):
    ...
    write_tif(f'lrm_{size}_9t_05.tif', lrm)

def tpi(z, radius_m):
    return (z - nanmean_filter(z, disk_kernel(radius_m / RES))).astype(np.float32)

tpi_05 = tpi(dem,  5.0); tpi_15 = tpi(dem, 15.0); tpi_51 = tpi(dem, 25.5)
write_tif('tpi_05_9t_05.tif', tpi_05)
write_tif('tpi_15_9t_05.tif', tpi_15)
write_tif('tpi_51_9t_05.tif', tpi_51)
gy, gx = np.gradient(tpi_15, RES)
write_tif('tpi_grad_mag_9t_05.tif', np.hypot(gx, gy).astype(np.float32))
write_tif('tpi_grad_dir_9t_05.tif', (np.degrees(np.arctan2(gx, -gy)) % 360).astype(np.float32))
...
print('\n=== openness (slowest step) ...')
op_pos, op_neg = openness(dem, L_cells=int(25/RES), cellsize=RES)
write_tif('openness_pos_9t_05.tif', op_pos)
write_tif('openness_neg_9t_05.tif', op_neg)
...
print('\n=== overview PNG ===')
panels = [
    ('hillshade_9t_05.tif',        'hillshade',           'gray',   (None,None)),
    ('dem_9t_05.tif',              'DEM (m)',             'terrain',(None,None)),
    ('slope_9t_05.tif',            'slope (deg)',         'magma',  (0, 30)),
    ('intensity_ground_9t_05.tif', 'ground intensity',    'cividis',(None,None)),
    ('ground_density_9t_05.tif',   'ground density',      'viridis',(0, 8)),
    ('chm_9t_05.tif',              'CHM (m)',             'Greens', (0, 30)),
    ('lrm_11_9t_05.tif',           'LRM 11',              'RdBu_r', (-0.6, 0.6)),
    ('lrm_25_9t_05.tif',           'LRM 25',              'RdBu_r', (-0.8, 0.8)),
    ('tpi_15_9t_05.tif',           'TPI 15 m',            'RdBu_r', (-0.5, 0.5)),
    ('openness_pos_9t_05.tif',     'openness_pos',        'viridis',(None,None)),
    ('openness_neg_9t_05.tif',     'openness_neg',        'viridis',(None,None)),
    ('local_relief_10_9t_05.tif',  'local relief (10 m)', 'magma',  (0, 3)),
]
...
fig.suptitle('output3 9-tile (4.5 km x 4.5 km) — 0.5 m derivative stack', fontsize=14)
fig.tight_layout(rect=[0, 0, 1, 0.98])
fig.savefig(DERIV/'tile_overview_9t_05.png', dpi=120, bbox_inches='tight')
plt.close(fig)
print('wrote tile_overview_9t_05.png')
