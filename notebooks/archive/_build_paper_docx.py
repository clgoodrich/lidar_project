...
add_fig(str(ROOT / 'Pennsylvania Map.png'), 'Distribution of documented oil and gas wells across Pennsylvania.')
add_fig(str(ROOT / 'Pennsylvania Map Abandoned.png'), 'Distribution of abandoned wells in Pennsylvania.')
...
add_fig(str(ROOT / 'region_with_trees.png'), 'Satellite imagery of the study area. Dense canopy hides all terrain features.')
...
add_fig(str(ROOT / 'region_without_trees.png'), 'LiDAR hillshade of the same area. With canopy removed, roads, pads, and pits are visible.')
...
add_fig(str(ROOT / 'Areas of Interest.png'), 'Study area tile footprints (white rectangles) on satellite imagery of western Pennsylvania.')
...
add_fig(str(ROOT / 'Pennsylvania AOI.png'), 'Study area with official PA DEP well locations by status category overlaid on LiDAR hillshade.')
...
add_fig(str(ROOT / 'Manual Picks.png'), 'Hillshade detail showing manual pit annotations (black stars) alongside official DEP well locations by status.')
...
add_fig(str(ROOT / 'close up errors.png'), 'Same area without manual annotations, showing DEP coordinate offset from visible pit features.')
add_fig(str(ROOT / 'orphaned with no clear wells.png'), 'DEP orphan records in an area with no clearly visible pit depressions.')
...
doc.save('WellSight_Paper_v5.docx')
print('Saved: WellSight_Paper_v5.docx')
