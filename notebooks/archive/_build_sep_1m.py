"""Build 1 m derivative stack from the 16-tile mosaic in
'data/files/seperate sections/'.  No ML; terrain derivatives only.

Outputs are tagged `_sep_1m` so they don't collide with any prior runs.

Tiles (4 x 4 grid, UTM 17N, 6 km x 6 km extent):
  X codes: 562, 564, 565, 567   ->  X 562500..568500
  Y codes: 464, 465, 467, 468   ->  Y 4464000..4470000
"""
...
LAS_OUT = DATA / 'sep_merged.las'
SFX = 'sep_1m'

# Mosaic extent (UTM 17N, m)
...
SRC = sorted(SRC_DIR.glob('*.laz'))
print(f'source tiles: {len(SRC)}')
...
if not LAS_OUT.exists():
    ...
    run_pipeline({'pipeline': stages}, 'merge_sep', timeout=3600)
    print(f'merged: {LAS_OUT.stat().st_size/1e9:.2f} GB')
else:
    print(f'{LAS_OUT} already exists, reusing')

# =============================================================================
# (2) DEM (TIN over ground) + DSM + CHM
# =============================================================================
print('\n=== DEM (TIN over ground) ===')
...
# Roughness 5-cell window (~5 m at 1 m)
WIN = 5
...
# LRM at scaled cell sizes (matches 0.5m convention at half scale)
...
fig.suptitle(f'separate sections — 6 km x 6 km — 1 m derivative stack', fontsize=14)
