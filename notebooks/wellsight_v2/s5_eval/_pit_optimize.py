...
def load_9t(split: str = "test"):
    """Load the 9t floor prob + argmax and the GT pit floors in the `split` region.

    Tuning MUST use split='val'; split='test' is reserved for the single frozen
    evaluation (pre-2026-07 runs tuned on test, inflating the reported F1)."""
...
def optimize(D):
    ...
    print(f"FROZEN cfg on TEST: f1={test['f1']} prec={test['prec']} rec={test['rec']} "
          f"tp={test['tp']} fp={test['fp']} fn={test['fn']} n={test['n']}")
...
def main():
    ...
    if args.apply_block:
        ...
        return 0
    D = load_9t("val")
    print(f"GT pit floors in 9t VAL region (tuning): {len(D['gt'])}")
    optimize(D)
    return 0
