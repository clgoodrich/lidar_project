"""Iter 02 — segmentation_models_pytorch U-Net with ImageNet-pretrained ResNet34 encoder.

Same data, training recipe, and TTA-inference as iter 01.  Only the model changes:
  smp.Unet(encoder_name='resnet34', encoder_weights='imagenet', in_channels=7, classes=3)

SMP handles non-3 input channels by averaging the pretrained first-conv weights across
channels and re-tiling, so the ImageNet features still transfer.

Outputs land under data/derivatives/9t/iterations/02_smp_pretrained/.
"""
...
from _pit_unet_v2 import FocalCE, PitTiles

ROOT = Path(r"C:\Users\colto\Documents\GitHub\lidar_project")
D = ROOT / "data" / "derivatives" / "9t"
ANN = ROOT / "data" / "derivatives" / "annotations" / "annotations_proj.gpkg"
OUTDIR = D / "iterations" / "02_smp_pretrained"; OUTDIR.mkdir(parents=True, exist_ok=True)
