"""Plat iter 01 — TTA + miou checkpoint (plat U-Net).

Mirror of the pit iter 01 changes applied to the binary plat segmentation:
  1) Save checkpoint by val plat IoU (the existing _plat_unet.py already does
     this — the criterion was correct from the start. Carried forward.)
  2) 8-fold TTA at inference (4 rotations x 2 flips, averaged softmax).

Output dir: data/derivatives/9t/iterations/plat_01_tta_miou/
"""
...
from _plat_unet import (
    UNet, FocalCE, PlatTiles, run_epoch,
    D, ANN, FEATURES, LABELS_PLAT, STATS, BLOCKS, MANIFEST,
    DEVICE, PATCH, N_CLASSES, N_CH
)

OUTDIR = D / "iterations" / "plat_01_tta_miou"; OUTDIR.mkdir(parents=True, exist_ok=True)


def tta_predict_batch(model, x):
    """8-fold TTA: 4 rotations x 2 flips, average softmax, inverting augs."""
    outs = []
    for k in range(4):
...
def infer_full(model, mu, sd):
    ...
    BATCH = 4  # smaller batch since TTA multiplies by 8
    buf_x, buf_pos = [], []
    def flush():
        if not buf_x: return
...
def main():
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if iou_plat > best_iou:
            ...
            torch.save({"state_dict": model.state_dict(), "mu": mu, "sd": sd,
                        ...
                        "iou_plat": iou_plat}, OUTDIR / "best.pt")
    ...
    print(f"\nLoaded best from ep {ck['epoch']} (plat IoU={ck['iou_plat']:.3f}). Running TTA inference...")
