"""Run full pipeline on 2 high-density orphan well tiles + McKean 5x5.

Tiles:
  616593 — 204 orphan/abandoned wells, 7km from training
  610594 — 163 orphan/abandoned wells, 11km west
  McKean 5x5 — derivatives already built, just needs XGB

Also overlays DEP well locations on the results for validation.
"""
...
# Test tiles (single LAZ + McKean 5x5)
TEST_TILES=[
    dict(code='616593',path=DATA/'USGS_LPC_PA_WesternPA_2019_D20_17TPF616593.laz',
         X0=616500.0,Y0=4593000.0,X1=618000.0,Y1=4594500.0,sfx='_616593_1m',
         label='204 orphan wells'),
    dict(code='610594',path=DATA/'USGS_LPC_PA_WesternPA_2019_D20_17TPF610594.laz',
         X0=610500.0,Y0=4594500.0,X1=612000.0,Y1=4596000.0,sfx='_610594_1m',
         label='163 orphan wells'),
    dict(code='mk5',path=None,  # derivatives already built
         X0=697000.0,Y0=4645000.0,X1=703500.0,Y1=4651500.0,sfx='_mk5_1m',
         label='McKean 5x5'),
]
for t in TEST_TILES:
...
# Load DEP wells
dep_shp='data/external/OilGasLocations_ConventionalUnconventional2026_04/OilGasLocations_ConventionalUnconventional2026_04.shp'
dep=gpd.read_file(dep_shp)
dep_orphan=dep[dep['WELL_STATU'].isin(['DEP Orphan List','DEP Abandoned List','Abandoned'])].to_crs(CRS)
dep_oxy=np.array([[g.x,g.y] for g in dep_orphan.geometry])
print(f'DEP orphan/abandoned wells loaded: {len(dep_orphan)}',flush=True)
...
def build_derivatives(tile):
    if tile['path'] is None:return  # already built
    sfx=tile['sfx'];X0,Y0,X1,Y1=tile['X0'],tile['Y0'],tile['X1'],tile['Y1']
    W,H,T=tile['W'],tile['H'],tile['T'];src=str(tile['path'])
    if(DERIV/f'dem{sfx}.tif').exists():
        print(f'{tile["code"]}: derivatives exist, skipping',flush=True);return
    print(f'\n=== building derivatives: {tile["code"]} ===',flush=True)
    ...
    t15=(dem-nmf(dem,dk(15/RES))).astype(np.float32);write_tif(f'tpi_15{sfx}.tif',t15,T,W,H)
    ...
    print(f'  done: {tile["code"]}',flush=True)

# Build derivatives for single tiles
...
print(f'templates: clusters={np.bincount(km.labels_).tolist()}',flush=True)
...
print(f'  features: {X_train.shape}  pos: {y_train.sum()}',flush=True)
...
def oof_fit(make_clf):
    ...
    return oof
p_oof=(oof_fit(make_xgb)+oof_fit(make_lgb))/2
iso=IsotonicRegression(out_of_bounds='clip').fit(p_oof,y_train)
print(f'  OOF PR-AUC: {average_precision_score(y_train,p_oof):.4f}',flush=True)
...
for tile in TEST_TILES:
    ...
    Ht,Wt=tile['H'],tile['W']
    print(f'\n=== {tile["code"]}: {tile["label"]} ===',flush=True)
    ...
    print(f'  candidates: {len(peaks)} in {time.time()-t0:.1f}s',flush=True)
    ...
    for i,(x,y) in enumerate(zip(xs,ys)):
        ...
        if f is not None:feat_list.append(f);keep_xy.append((x,y))
        if(i+1)%50000==0:print(f'  {i+1}/{len(xs)} ({time.time()-t0:.0f}s)',flush=True)
    ...
    print(f'  features: {X_test.shape} in {time.time()-t0:.1f}s',flush=True)
    ...
    # DEP wells in this tile
    dep_in=dep_orphan[
        (dep_orphan.geometry.x>=X0t)&(dep_orphan.geometry.x<=X1t)&
        (dep_orphan.geometry.y>=Y0t)&(dep_orphan.geometry.y<=Y1t)]
    dep_xy_t=np.array([[g.x,g.y] for g in dep_in.geometry]) if len(dep_in) else np.empty((0,2))

    print(f'  wrote {fname} ({len(gdf)} rows)  DEP wells in tile: {len(dep_in)}')
    for thr in [0.30,0.50,0.70,0.90]:
        n=int((gdf['proba']>=thr).sum())
        print(f'    proba>={thr}: {n}')

    # Overview with DEP wells overlay
    hs=read_tif(f'hillshade{sfx}.tif')
    fig,ax=plt.subplots(figsize=(14,14))
    ax.imshow(hs,cmap='gray',extent=[X0t,X1t,Y0t,Y1t])
    if len(dep_xy_t):
        ax.scatter(dep_xy_t[:,0],dep_xy_t[:,1],s=25,facecolors='none',edgecolors='cyan',
                   linewidths=0.8,label=f'DEP orphan/abandoned ({len(dep_in)})')
    for thr,color,sz in [(0.50,'yellow',30),(0.70,'orange',50),(0.90,'red',70)]:
        ...
        if len(sub):
            ax.scatter(sub.geometry.x,sub.geometry.y,s=sz,c=color,edgecolors='black',
                       linewidths=0.3,label=f'proba {thr:.1f}+ (n={len(sub)})')
    ax.set_title(f'{tile["code"]} ({tile["label"]})\nDEP wells: {len(dep_in)}',fontsize=12)
    ax.legend(loc='lower left',fontsize=9)
    ax.set_xlim(X0t,X1t);ax.set_ylim(Y0t,Y1t)
    fig.savefig(DERIV/f'pit_{tile["code"]}_dep_overlay.png',dpi=130,bbox_inches='tight')
    plt.close(fig)
    print(f'  wrote pit_{tile["code"]}_dep_overlay.png')
